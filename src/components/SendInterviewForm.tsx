...
export function SendInterviewForm({ agents, respondents }: { agents: Agent[]; respondents: Respondent[] }) {
  ...
  if (!open) {
    return (
      <ClayButton onClick={() => setOpen(true)} className="mb-5">
        + Send interview
      </ClayButton>
    );
  }
  ...
  function submit() {
    ...
    startTransition(async () => {
      await sendInterview(agentId, Array.from(selected));
      setSelected(new Set());
      setOpen(false);
      router.refresh();
    });
  ...
  return (
    <div className="mb-5 space-y-3 rounded-xl border border-(--color-line-soft) bg-(--color-raised) p-4">
      ...
      <div className="flex gap-2 pt-0.5">
        <ClayButton onClick={submit} disabled={pending || !agentId || selected.size === 0} className="!py-1.5 !text-[12.5px]">
