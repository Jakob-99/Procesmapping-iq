/*
  "Spørg på tværs" — institutionel hukommelse over ALLE interviews i et
  engagement. Stateless: bygger kontekst af noter + gemte citater hver gang,
  ingen chatlog gemmes. Svaret citerer sine kilder, så man kan klikke videre
  til det interview det kommer fra.
*/
...
export async function answerCrossQuery(
  ...
  if (notes.length === 0 && quotes.length === 0) {
    throw new Error("Der er ikke noget at søge i endnu — gennemfør nogle interviews først.");
  }

  const context = [
    ...notes.map(
      (n) =>
        `NOTE [interviewId=${n.interviewId}] (${n.category}, "${n.interview.interviewAgent.name}" / ${n.interview.respondent.name}): ${n.content}`,
    ),
    ...quotes.map(
      (q) =>
        `CITAT [interviewId=${q.interviewId}] ("${q.interview.interviewAgent.name}" / ${q.interview.respondent.name}): "${q.text}"`,
    ),
  ].join("\n");
  ...
  return generateJson<CrossQueryResult>({
    ...
    prompt: `--- Noter og citater fra interviews ---\n${context}\n\n--- Spørgsmål ---\n${question}`,
